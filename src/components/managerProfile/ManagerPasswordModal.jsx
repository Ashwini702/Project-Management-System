// src/components/managerProfile/ManagerPasswordModal.jsx
import React, { useState } from 'react';
import { FiX, FiEye, FiEyeOff } from 'react-icons/fi';

const ManagerPasswordModal = ({ show, onClose, onSubmit }) => {
      ...
      <form onSubmit={handleSubmit}><div className="modal-body">
        {['current', 'newPass', 'confirm'].map(f => (
          <div className="mb-3" key={f}>
            <label className="form-label">{f === 'current' ? 'Current Password *' : f === 'newPass' ? 'New Password *' : 'Confirm Password *'}</label>
