// src/components/reports/ReportChartBox.jsx
import React from 'react';

const ReportChartBox = ({ title, data, type = 'bar', total, subtitle }) => {
        ...
        {type === 'bar' && data.map((item, i) => (
          <div key={i} className="chart-bar-row">
            ...
            <div className="chart-bar-wrapper">
              <div className="chart-bar" style={{ width: `${item.percentage || (item.value / (total || 100)) * 100}%`, backgroundColor: item.color || colors[i] }}></div>
            </div>
