// src/components/clientProfile/ClientSecuritySettings.jsx
import React, { useState } from 'react';
import { FiLogOut } from 'react-icons/fi';
import { securitySettingsData, loginHistory, activeSessions } from '../../data/clientProfileData';

const ClientSecuritySettings = ({ onChangePassword, onAlert }) => {
  ...
  return (
    <div>
      <div className="cprof-info-card mb-3">
        ...
        <div className="cprof-toggle-row">
      ...
      <div className="cprof-info-card mb-3">
        ...
        {activeSessions.map(s => (
          <div key={s.id} className={`cprof-session-item ${s.current ? 'current' : ''}`}>
            <div><strong>{s.device}</strong><span>{s.browser} • {s.location}</span><span className="cprof-session-time">{s.lastActive}</span></div>
      ...
      <div className="cprof-info-card">
        ...
        <div className="table-responsive">
          <table className="table cprof-table">
            <thead><tr><th>Date/Time</th><th>IP</th><th>Device</th><th>Browser</th><th>Location</th><th>Status</th></tr></thead>
            <tbody>{loginHistory.map(l => <tr key={l.id}><td>{l.dateTime}</td><td><code>{l.ipAddress}</code></td><td>{l.device}</td><td>{l.browser}</td><td>{l.location}</td><td><ClientStatusBadge status={l.status === 'Success' ? 'Active' : 'Inactive'} /></td></tr>)}</tbody>
          </table>
        </div>
      </div>
    </div>
