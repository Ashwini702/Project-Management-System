...
import { meetingTypes, meetingStatuses, meetingModes, projectsForMeetings, clientsForMeetings, participantsList } from '../../data/meetingData';
...
const MeetingFormModal = ({ show, onClose, onSubmit, editMeeting }) => {
  ...
  const addAgenda = () => { if (agendaInput.trim()) { setFormData(p => ({ ...p, agenda: [...p.agenda, { id: Date.now(), text: agendaInput.trim(), status: 'Pending' }] })); setAgendaInput(''); } };
  ...
  return (
    <div className="modal-overlay dashboard-form-overlay"><div className="modal-dialog modal-lg dashboard-form-dialog"><div className="modal-content dashboard-form-modal">
      <div className="modal-header dashboard-form-header"><h5 className="modal-title">{editMeeting ? 'Edit Meeting' : 'Schedule Meeting'}</h5><button className="modal-close-btn" onClick={onClose}><FiX /></button></div>
      <form className="dashboard-form" onSubmit={handleSubmit}><div className="modal-body dashboard-form-body"><div className="row dashboard-form-grid">
        <div className="col-md-12 mb-3"><label className="form-label">Meeting Title *</label><input type="text" name="title" className={`form-control ${errors.title ? 'is-invalid' : ''}`} value={formData.title} onChange={handleChange} />{errors.title && <div className="invalid-feedback">{errors.title}</div>}</div>
        <div className="col-md-6 mb-3"><label className="form-label">Meeting Type *</label><select name="type" className={`form-select ${errors.type ? 'is-invalid' : ''}`} value={formData.type} onChange={handleChange}>{meetingTypes.map(t => <option key={t} value={t}>{t}</option>)}</select>{errors.type && <div className="invalid-feedback">{errors.type}</div>}</div>
        <div className="col-md-6 mb-3"><label className="form-label">Project *</label><select name="project" className={`form-select ${errors.project ? 'is-invalid' : ''}`} value={formData.project} onChange={handleChange}><option value="">Select Project</option>{projectsForMeetings.map(p => <option key={p} value={p}>{p}</option>)}</select>{errors.project && <div className="invalid-feedback">{errors.project}</div>}</div>
        ...
        <div className="col-md-3 mb-3"><label className="form-label">Start Time *</label><input type="time" name="startTime" className={`form-control ${errors.startTime ? 'is-invalid' : ''}`} value={formData.startTime} onChange={handleChange} />{errors.startTime && <div className="invalid-feedback">{errors.startTime}</div>}</div>
        <div className="col-md-3 mb-3"><label className="form-label">End Time *</label><input type="time" name="endTime" className={`form-control ${errors.endTime ? 'is-invalid' : ''}`} value={formData.endTime} onChange={handleChange} />{errors.endTime && <div className="invalid-feedback">{errors.endTime}</div>}</div>
        ...
        {(formData.mode === 'Online' || formData.mode === 'Hybrid') && <div className="col-md-6 mb-3"><label className="form-label">Meeting Link</label><input type="text" name="meetingLink" className="form-control" value={formData.meetingLink} onChange={handleChange} placeholder="https://meet.google.com/..." /></div>}
        {(formData.mode === 'Offline' || formData.mode === 'Hybrid') && <div className="col-md-6 mb-3"><label className="form-label">Location</label><input type="text" name="location" className="form-control" value={formData.location} onChange={handleChange} placeholder="Conference Room A" /></div>}
        ...
        <div className="col-md-12 mb-3"><label className="form-label">Participants *</label><div className="participants-select-grid">{participantsList.map(p => <div key={p.id} className={`participant-chip ${formData.participants.includes(p.name) ? 'selected' : ''}`} onClick={() => toggleParticipant(p.name)}>{p.name}</div>)}</div>{errors.participants && <div className="invalid-feedback d-block">{errors.participants}</div>}</div>
        <div className="col-md-12 mb-3"><label className="form-label">Agenda</label><div className="agenda-input-row"><input type="text" className="form-control" value={agendaInput} onChange={(e) => setAgendaInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addAgenda())} placeholder="Add agenda item" /><button type="button" className="btn btn-sm btn-primary" onClick={addAgenda}><FiPlus /></button></div><div className="agenda-list mt-2">{formData.agenda.map(a => <div key={a.id} className="agenda-chip">{a.text} <button type="button" onClick={() => removeAgenda(a.id)}><FiTrash2 /></button></div>)}</div></div>
      ...
      <div className="modal-footer dashboard-form-footer"><button type="button" className="btn btn-light" onClick={onClose}>Cancel</button><button type="submit" className="btn btn-primary">{editMeeting ? 'Update Meeting' : 'Save Meeting'}</button></div></form>
    </div></div></div>
  );
};
