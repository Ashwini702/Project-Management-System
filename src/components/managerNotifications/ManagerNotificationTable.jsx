// src/components/managerNotifications/ManagerNotificationTable.jsx
...
const ManagerNotificationTable = ({ notifications, onView, onToggleRead, onToggleImportant, onArchive, onDelete }) => (
  <div className="table-responsive">
    <table className="table mgn-table">
        ...
        {notifications.length === 0 ? <tr><td colSpan="9" className="text-center py-5"><p className="text-muted">No notifications found</p></td></tr> :
          notifications.map(n => (
            <tr key={n.id} className={!n.isRead ? 'row-unread' : ''}>
              <td><div><span className="mgn-name">{n.title}</span><small className="d-block text-muted">{n.message.substring(0, 50)}...</small></div></td>
              <td><ManagerNotificationTypeBadge type={n.type} /></td><td>{n.relatedProject}</td><td>{n.relatedTo}</td>
              <td><span className={`priority-dot priority-${n.priority?.toLowerCase()}`}></span> {n.priority}</td>
              <td><ManagerNotificationStatusBadge status={n.status} isRead={n.isRead} /></td>
              <td>{n.senderName}</td><td>{n.createdAt}</td>
              <td><div className="action-buttons">
                <button className="action-btn-icon view-btn" onClick={() => onView(n)}><FiEye /></button>
                <button className="action-btn-icon" onClick={() => onToggleRead(n)} title={n.isRead ? 'Mark Unread' : 'Mark Read'}>{n.isRead ? <FiMail /> : <FiCheck />}</button>
...
export default ManagerNotificationTable;